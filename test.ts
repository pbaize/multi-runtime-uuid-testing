...
//     const run = (app: DecoratedApp, i: number) => app.run().then(() =>
//     console.log('successful run of app ', i)).then(() => true).catch((e) => {
...
for (let count = 2; count < 11; count++) {
  for (let run = 1; run < 101; run++) {
    test(`Simultaeneous run ${count} apps # ${run}`, async t => {
      const run = (app: DecoratedApp, i: number) =>
          app.run()
              .then(() => console.log('successful run of app ', i))
              .then(() => true)
              .catch((e) => {
                console.log(e.message);
                return false;
      ...
      // tslint:disable-next-line:ban
      const apps =
          await Promise.all(Array(count).fill(null).map(() => createApp()));
      const successes = await Promise.all(apps.reverse().map(run));
      t.pass();
      // t.true(successes.filter(id).length === 1);
    });
  }
